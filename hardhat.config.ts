...
task("approve", "approve contract")
  ...
  .setAction(async ({ token, spender, rpc, pk }, { ethers, run, network }) => {
    await run("compile");
...
task("deploy-upgrade", "deploy upgrade contract")
  ...
  .setAction(async ({ name, symbol, supply, owner }, { run }) => {
    await run("upgrade", {
      contract: "ERC20MintablePauseableUpgradeable",
      params: JSON.stringify([name, symbol, supply, owner]),
    });
  });
// npx hardhat deploy-system --network metermain
task("deploy-system", "deploy system contract").setAction(
  async ({}, { ethers, run, network, upgrades }) => {
    await run("upgrade", {
      contract: "MeterERC20Upgradeable",
      params: "[]",
    });
    await run("upgrade", {
...
task("update", "update contract")
  ...
  .setAction(async ({ contract, address }, { ethers, run, upgrades }) => {
    await run("compile");
...
task("setBlackList", "set BlackList")
  ...
  .setAction(async ({ account }, { ethers, run, network }) => {
    await run("compile");
    const signers = await ethers.getSigners();
...
// npx hardhat pause
task("pause", "pause contract").setAction(
...
// npx hardhat unpause
task("unpause", "unpause contract").setAction(
  async (taskArgs, { ethers, run, network }) => {
    await run("compile");
    const signers = await ethers.getSigners();
...
task("info", "token info").setAction(async ({}, { ethers, run, network }) => {
  await run("compile");
  const signers = await ethers.getSigners();
...
task("permit", "revoke minter Role")
  ...
  .setAction(async ({ spender, value }, { ethers, run, network }) => {
    await run("compile");
    const signers = await ethers.getSigners();
    const tokenAddr = getContract(
      network.name,
      "ERC20MintablePauseableUpgradeable"
    );
    ...
    const chainId =
      network.name == "ganache" ? 1 : await signers[0].getChainId();
...
task("cf", "contracts factory").setAction(
  async ({}, { ethers, run, network }) => {
    ...
    let tx =
      "";
    let receipt = await ethers.provider.sendTransaction(tx);
    console.log(await receipt.wait());
    let BYTECODE =
      "";
...
task("deployRouter", "gas less swap").setAction(
  async ({}, { ethers, run, network }) => {
    await run("compile");
    const signers = await ethers.getSigners();
    // Testnet
...
task("dr2", "gas less swap").setAction(async ({}, { ethers, run, network }) => {
  await run("compile");
  const signers = await ethers.getSigners();
  // Testnet
  ...
  console.log(
    "getAmountsOut:",
    await router.getAmountsOut("1000000000000000000")
  );
});

// npx hardhat deployRouter --network metermain
task("metermaker", "deploy MeterMaker").setAction(
  async ({}, { ethers, run, network }) => {
    await run("compile");
    const signers = await ethers.getSigners();
    // Mainnet
...
task("gasLess", "gas less swap").setAction(
  async ({}, { ethers, run, network }) => {
    await run("compile");
    const signers = await ethers.getSigners();
    let pair = "0x87d244897695a5a0481057f217dbadda5c8d6a7e"; // MeterSwap LP Token (MLP:MTR-MTRG)
...
task("gl2", "gas less swap").setAction(
...
task("deploy-simple", "deploy contract")
  ...
  .addParam("symbol", "Token symbol")
  .addParam("supply", "Token initialSupply require decimal")
  .setAction(async ({ name, symbol, supply }, { ethers, run, network }) => {
    await run("compile");
    const signers = await ethers.getSigners();
...
task("de", "deploy Exchange").setAction(
